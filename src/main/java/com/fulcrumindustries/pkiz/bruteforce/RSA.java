...
public class RSA {
    ...
    /**
     * Um tamanho típico para n é de 1024-bits, ou 309 dígitos decimais. Ou
     * seja, n é menor do que 2^1024. Além disso, o tamanho da mensagem M deve
     * ser menor do que o valor de n. [1]
     */
    ...
    /**
     * Essa função define o número de primos relativos encontrados menores que
     * "n" [1].
     *
     * Quando a função recebe um número primo "n", como todos números de 1 até
     * n-1 são primos relativos de "n", então totient(n) para "n" primo é igual
     * à "n-1" [1].
     *
     * Quando a função recebe um número não primo de "n", precisamos listar
     * todos os números que são primos relativos de "n". Exemplo, a para
     * totient(35) os primos relativos são:
     * [1,2,3,4,6,8,9,11,12,13,16,17,18,19,22,23,24,26,27,29,31,32,33,34] logo
     * totient(35) = 24 [1].
     *
    ...
    /**
     * Na prática, as escolhas comuns de e são 3, 5, 17, 257 e 65537 (2^16 + 1).
     * Esses números particulares são escolhidos porque eles são primos e fazem
     * a operação de exponenciação modular ser rápida, tendo somente dois bits
     * de valor 1. [3]
     *
     * Esses cinco números são os cinco primeiros números de Fermat, referidos
     * como F0 até F4, onde Fx = 2^(2^x) + 1. Entretanto somente os cinco
     * primeiros números são primos, o número F5 já não é primo: F5 = 4294967297
     * = 641 × 6700417.
     */
    ...
    /**
     * Gera um número aleatório limitado a 2^bits com exatamente bits
     * algarismos. [2]
     *
     */
    ...
    /**
     * O teorema de fermat afirma que: Se "p" é primo e "a" é um inteiro
     * positivo não divisível por "p", "a" < "p" então [1]:
     *
     * a^(p-1) = 1 mod p (8.2)
     *
     * Essa formula funciona porque todos elementos que vão de (1, 2, ..., p-1)
     * não são divísiveis por p, logo se fizermos X = {a mod p, 2a mod p, 3a mod
     * p, ... (p-1) mod p} se "a" e "p" forem primos entre sí todos esses
     * valores irão resultar em 1, por que nem "a" e nenhum elemento de (1, 2,
     * ... p-1) são divisível por "p". Porém o teorema de fermat é simplicado da
     * seguinte forma [1]:
     *
     * a * 2a * ... * (p-1)a = [(1 * 2 * ... * (p-1)] mod p a^(p-1) (p-1)! =
     * (p-1)! mod p
     *
     * Como nenhum elemento de (1,2,...,p-1) é divível por "p" nem mesmo o
     * produto, pode-se simplicar para [1]:
     *
     * a^(p-1) = 1 mod p
     *
     */
    ...
    /**
     * Esse teste se apoia em duas propriedades para testar se um número é primo
     * [1]. 1) A primeira propriedade é que a^2 mod p = 1 se e somente se: - a
     * mod p = 1 - a mod p = -1 mod p = p - 1, análogo a girar o relógio ao
     * contrário. 2) A segunda propriedade é baseada em n-1 = 2^k * q - No caso
     * de 1 positivo: a^q = 1 mod p - No caso de 1 negativo: algum a^q, a^(2q),
     * a^3q, a^((2^k)q) será equivalente a -1. Logo a^((2^j)*q) mod n = p-1.
     *
     * Assim se n é primo, para a lista de resíduso definida [a^q, a^(q*2), ...,
     * a^(q*2^(k-1)), a^(q*2^k)] módulo de n, ou o primeiro elemento da lista
     * será primo ou algum elemento nessa lista será igual a (n-1). Essas duas
     * possiblidades explicam a probabilidade menor que 1/4 em determinar um
     * número composto como primo.
     */
    private boolean isPrimeMillerRabin(BigInteger n, int k, BigInteger q) {
        ...
        if (a.modPow(q, n).compareTo(new BigInteger("1")) == 0) {
            return true; // Exponenciação modular = O(n^3) [6]
        }
        for (int j = 0; j < k; j++) {
            // a^((2^j)*q) mod n
            ...
            if (mod.compareTo(n.subtract(new BigInteger("1"))) == 0) {
                return true; // Subtract e compare = O(n) [6]
            }
        }
        return false;
    }
